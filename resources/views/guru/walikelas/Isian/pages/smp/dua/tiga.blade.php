<table border="1" style="width: 100%;border-collapse: collapse;">
	<tr>
		...
		@if($kd->count()!=0)
		@foreach($kd as $k=>$v)
		<th onclick="nama_kolom('bg-lime','KD {{$k+1}}','{{$v->isi}}','npts_{{$k+1}}')">
			<i class="fa fa-comment"></i> KD {{($k+1)}}
			<div style="display: none;border: 1px solid black;margin: 5px;padding: 5px;font-size: 12px;" class="tooltip123 bg-dark" id="npts_{{$k+1}}">
				{{$v->isi}}
			</div>
		</th>
		@endforeach
	...
	<tr style="@if($k%2==0) background: #eee @endif">
		...
		<td>
			...
			<input type="number" name="npts_{{$s->id_siswa}}[]" value="{{(isset($s->nilai[$v->id_kd]) && $s->nilai[$v->id_kd]!=0) ? $s->nilai[$v->id_kd] : ''}}">
		</td>
		@endforeach
		@else
