<table border="1" style="width: 100%;border-collapse: collapse;">
	<tr>
		...
		@if($kd->count()!=0)
		@foreach($kd as $k=>$v)
		<th onclick="nama_kolom('bg-lime','KD {{$k+1}}','{{$v->isi}}','proyek_{{$k+1}}')">
	...
	<tr style="@if($k%2==0) background: #eee @endif">
		...
		<td>
			...
			<input type="number" name="proyek_{{$s->id_siswa}}[]" value="{{(isset($s->nilai[$v->id_kd]) && $s->nilai[$v->id_kd]!=0) ? $s->nilai[$v->id_kd] : ''}}">
		</td>
		@endforeach
		@else
		<td>..:: KD tidak disetting ::..</td>
		@endif
		<td><a href="javascript:void(0)" class="btn btn-sm btn-primary" onclick="simpan_nilai('{{$s->id_siswa}}','proyek','4')">Simpan</a></td>
	</tr>
	@endforeach
	@else
	<tr>
