...
<div class="row">
		...
		<div class="row">
			...
			<div class="col-lg-3 col-md-3 col-sm-12 xol-xs-12">
				<div class="card card-info">
					<div class="card-header" style="cursor: pointer;" onclick="nama_kolom('bg-lime','KD {{$k+1}}','{{$v->kd_isi}}','uh_{{$k+1}}')">
						<i class="fa fa-comment"></i> KD {{($k+1)}}
					...
					<div class="card-body" style="background: #ccc">
						<input type="number" class="form-control" name="uh[]" value="{{(!empty($mengajar)) ? $mengajar->$kolom : ''}}" @if($mengajar->islock_uts==true) readonly @endif>
					</div>
				</div>
			</div>
			@endforeach
	...
	<div style="width: 100%">
		<a style="width: 100%" href="javascript:void(0)" class="btn btn-primary btn-lg" @if($mengajar->islock_uts==true) onclick="alert('Pengisian nilai sudah dikunci')" @else onclick="simpankd('uh')" @endif>Simpan</a>
	</div>
</div>